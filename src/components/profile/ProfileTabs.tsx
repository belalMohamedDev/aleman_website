...
export function ProfileTabs({ activeTab, onChangeTab, counts }: ProfileTabsProps) {
  const tabs: Array<{ id: ProfileTabType; label: string; icon: any; count: number }> = [
    { id: 'orders', label: 'طلباتي', icon: PackageIcon, count: counts.orders },
    { id: 'merchant-orders', label: 'طلبات العملاء (صغار التجار)', icon: StoreIcon, count: counts.merchantOrders },
    { id: 'customers', label: 'عملائي', icon: UsersIcon, count: counts.customers },
      ...
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={`flex shrink-0 items-center gap-2.5 rounded-2xl px-5 py-3 text-sm font-extrabold transition-all ${
              isActive
