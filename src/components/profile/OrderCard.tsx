...
export function OrderCard({ order, onViewDetails, onCancelOrder, showCustomerName }: OrderCardProps) {
  ...
  const getStatusDisplay = () => {
    ...
    if (s === OrderStatus.Confirmed || s === 2) {
      return { text: t(ui.orders.statuses.confirmed), bg: 'bg-sky-50 text-sky-700 border-sky-200' };
    }
    if (s === OrderStatus.Preparing || s === 3) {
      return { text: t(ui.orders.statuses.preparing), bg: 'bg-indigo-50 text-indigo-700 border-indigo-200' };
    }
    if (s === OrderStatus.OutForDelivery || s === 4) {
      return { text: t(ui.orders.statuses.outForDelivery), bg: 'bg-purple-50 text-purple-700 border-purple-200' };
    }
    if (s === OrderStatus.ReadyForPickup || s === 5) {
      return { text: t(ui.orders.statuses.readyForPickup), bg: 'bg-sky-50 text-sky-700 border-sky-200' };
    }
    if (s === OrderStatus.Completed || s === OrderStatus.Delivered || s === 6) {
      return { text: t(ui.orders.statuses.completed), bg: 'bg-emerald-50 text-emerald-700 border-emerald-200' };
    }
    if (s === OrderStatus.Cancelled || s === 7) {
      return { text: t(ui.orders.statuses.cancelled), bg: 'bg-rose-50 text-rose-700 border-rose-200' };
    }
    if (s === OrderStatus.RejectedByMerchant || s === 10) {
      return { text: t(ui.orders.statuses.rejectedByMerchant), bg: 'bg-rose-50 text-rose-700 border-rose-200' };
    }
    if (s === OrderStatus.RejectedByAdmin || s === 11) {
      return { text: t(ui.orders.statuses.rejectedByAdmin), bg: 'bg-rose-50 text-rose-700 border-rose-200' };
    }
    if (s === OrderStatus.Refunded || s === 13) {
      return { text: t(ui.orders.statuses.refunded), bg: 'bg-slate-100 text-slate-700 border-slate-200' };
    }
    return {
  ...
  const getWeightDisplay = () => {
    ...
    if (order.totalWeightKg !== undefined && order.totalWeightKg !== null && order.totalWeightKg > 0) {
      ...
      return tons >= 0.001 ? `${tons} ${t(ui.cart.tonUnit)}` : `${order.totalWeightKg} ${t(ui.cart.kgUnit)}`;
    }
    return `0 ${t(ui.cart.tonUnit)}`;
  };
  ...
  const formattedTotal = `${Number(order.totalAmount || 0).toFixed(1)} ${t(ui.common.currency)}`;
  ...
  return (
    <div
      ...
      className={`group relative rounded-2xl border border-slate-200/90 bg-white p-4 shadow-xs transition-all duration-200 hover:-translate-y-1 hover:shadow-md hover:border-emerald-500/40 active:scale-[0.99] flex flex-col justify-between h-full cursor-pointer ${isRtl ? 'text-right' : 'text-left'} select-none`}
    >
      <div className="space-y-3">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          {isBankTransfer ? (
            <span className="inline-flex items-center gap-1 rounded-lg bg-purple-50 text-purple-700 border border-purple-200/70 px-2 py-0.5 text-[11px] font-bold">
              ...
              <span>{t(ui.checkout.bankTransfer)}</span>
            </span>
          ) : (
            <span className="inline-flex items-center gap-1 rounded-lg bg-slate-100 text-slate-700 border border-slate-200 px-2 py-0.5 text-[11px] font-bold">
              ...
              <span>{t(ui.checkout.cashOnDelivery)}</span>
            </span>
          )}
          ...
          {order.orderType === OrderType.Delivery ? (
            <span className="inline-flex items-center gap-1 rounded-lg bg-sky-50 text-sky-700 border border-sky-200/70 px-2 py-0.5 text-[11px] font-bold">
              ...
              <span>{t(ui.checkout.deliveryToFarm)}</span>
            </span>
          ) : (
            <span className="inline-flex items-center gap-1 rounded-lg bg-amber-50 text-amber-700 border border-amber-200/70 px-2 py-0.5 text-[11px] font-bold">
              ...
              <span>{t(ui.checkout.pickupFromFactory)}</span>
            </span>
          )}
          ...
          {showCustomerName && order.customerName && (
            <span className="inline-flex items-center gap-1 rounded-lg bg-emerald-50/80 text-[#234c2e] border border-emerald-200/60 px-2 py-0.5 text-[11px] font-bold truncate max-w-full">
              <span>{t(ui.profile.clientName)}</span>
        ...
        <div className="rounded-xl bg-slate-50/90 p-2.5 sm:p-3 border border-slate-100/90 flex items-center justify-between gap-2">
          <div className="min-w-0">
            <span className="block text-[11px] font-bold text-slate-400">{t(ui.profile.totalVolume)}</span>
          ...
          <div className="text-left shrink-0">
            <span className="block text-[11px] font-bold text-slate-400">{t(ui.orders.paymentStatusLabel)}</span>
      ...
      {/* Bottom Row: Optional Cancel */}
      {isPending && onCancelOrder && (
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation();
            if (window.confirm(t(ui.orders.cancelConfirm))) {
              onCancelOrder(order.id);
            }
          }}
          className="inline-flex items-center gap-1 rounded-lg px-2 py-1 text-[11px] font-bold text-red-600 hover:bg-red-50 transition cursor-pointer mt-2 self-start"
        >
